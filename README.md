...
2. Verify `openclaw.json` exists and has real tokens (fails if missing or has placeholders)
...
| `OPENCLAW_MONITOR_RUN_CANARY` | `1` | Slack canary probe enabled by default (posts as `SLACK_USER_TOKEN` and waits for a thread reply). Set to `0` to disable. |
...
| `openclaw.json.redacted` | Template for required config - copy to `openclaw.json` and fill in tokens |
