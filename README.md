...
> *ai.smartclaw3-18):** This repo now includes an automated daily harness engineering analyzer that runs at 9am via launchd.
...
export JLEECHAN_SLACK_USER_ID="<YOUR_SLACK_USER_ID>"  # your Slack user ID
export OPENCLAW_BOT_USER_ID="<YOUR_BOT_USER_ID>"        # openclaw bot user ID

# ── Slack channels (change if using a different Slack workspace) ──────────────
export AGENTO_CHANNEL="<AGENTO_CHANNEL_ID>"           # #ai-general — agento dispatch
...
|----------|-------|-------------|
| `JLEECHAN_SLACK_USER_ID` | `<YOUR_SLACK_USER_ID>` | Your Slack user ID. Scripts use this to identify messages from you. |
| `OPENCLAW_BOT_USER_ID` | `<YOUR_BOT_USER_ID>` | openclaw bot user ID. Gateway ignores its own messages using this. |
...
| `#smartclaw` | `<CHANNEL_ID>` |
| `#novel` | `<CHANNEL_ID>` |
| `#antig-orchestrator` | `<CHANNEL_ID>` |
| `#ralph-status` | `<CHANNEL_ID>` |
| `#disk-usage-alerts` | `<CHANNEL_ID>` |
...
Plist templates are under `launchd/` using the `smartclaw.schedule.*.plist.template` naming pattern.
