...
# Staging gateway (18810) uses ~/.openclaw/openclaw.staging.json.
# Architecture: ~/.openclaw/ = staging (18810), ~/.openclaw_prod/ = production (18789)
...
if [[ -n "${OPENCLAW_STAGING_CONFIG:-}" ]]; then
    ...
    CONFIG_FILE="$HOME/.openclaw_prod/openclaw.json"
elif [[ "$PORT" == "18810" ]]; then
    CONFIG_FILE="$HOME/.openclaw/openclaw.staging.json"
else
    echo "  WARN: Unknown port $PORT — defaulting to staging config (~/.openclaw/openclaw.staging.json)"
    CONFIG_FILE="$HOME/.openclaw/openclaw.staging.json"
fi
...
NODE_BIN="${OPENCLAW_NODE_BIN:-$(launchctl print gui/$(id -u)/ai.openclaw.gateway 2>/dev/null | grep -oE '/[^ ]*bin/node' | head -1 || echo '/Users/jleechan/.nvm/versions/node/v22.22.0/bin/node')}"
BETTER_SQLITE_PATH="$HOME/.openclaw/extensions/openclaw-mem0/node_modules/better-sqlite3"
...
SDK_VERSION=$(npm ls @agentclientprotocol/sdk --prefix "$HOME/.openclaw" 2>/dev/null | grep agentclientprotocol | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' || echo "")
if [[ -z "$SDK_VERSION" ]]; then
    ...
    SDK_VERSION=$(node -e "try { const p = require(require.resolve('@agentclientprotocol/sdk/package.json', {paths:['/opt/homebrew/lib/node_modules/openclaw','$(echo ~)/.openclaw']})); console.log(p.version); } catch(e) { console.log(''); }" 2>/dev/null || echo "")
fi
if [[ -z "$SDK_VERSION" ]]; then
...
# Derive state dir from port: prod port (~/.openclaw_prod), else staging (~/.openclaw)
if [[ "$PORT" == "$PROD_PORT" ]]; then
    SESSION_DIR="$HOME/.openclaw_prod/agents/main/sessions"
else
...
if [[ "$PORT" == "$PROD_PORT" ]]; then
    AUTH_DIR="$HOME/.openclaw_prod/agents/main/agent"
else
