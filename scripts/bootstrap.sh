...
# Canonical AO config lives under ~/.openclaw/ (tracked harness root). ~/agent-orchestrator.yaml
...
ensure_agent_orchestrator_symlink() {
    ...
    local canon_yaml="$HOME/.openclaw/agent-orchestrator.yaml"
...
if [[ "${1:-}" == "--eloop-skill-only" ]]; then
    if [[ -f "$REPO_ROOT/skills/jleechanclaw-eloop.md" ]]; then
        ...
        ln -sf "$REPO_ROOT/skills/jleechanclaw-eloop.md" "$HOME/.openclaw/skills/jleechanclaw-eloop.md"
        echo "OK: ~/.openclaw/skills/jleechanclaw-eloop.md -> $REPO_ROOT/skills/jleechanclaw-eloop.md"
    else
        echo "ERROR: missing $REPO_ROOT/skills/jleechanclaw-eloop.md" >&2
        exit 1
    fi
    exit 0
fi
...
# jleechanclaw custom eloop — AO orchestratorRules references ~/.openclaw/skills/jleechanclaw-eloop.md
if [[ -f "$REPO_ROOT/skills/jleechanclaw-eloop.md" ]]; then
    mkdir -p "$HOME/.openclaw/skills"
    ln -sf "$REPO_ROOT/skills/jleechanclaw-eloop.md" "$HOME/.openclaw/skills/jleechanclaw-eloop.md" \
        && echo "Symlink: ~/.openclaw/skills/jleechanclaw-eloop.md -> $REPO_ROOT/skills/jleechanclaw-eloop.md"
else
    echo "WARNING: $REPO_ROOT/skills/jleechanclaw-eloop.md missing — ~/.openclaw/skills/jleechanclaw-eloop.md not created (AO eloop path will be broken)" >&2
fi

# Agent Orchestrator config — symlink ~/agent-orchestrator.yaml → ~/.openclaw/agent-orchestrator.yaml (canonical)
...
if command -v gog &>/dev/null; then
    ...
    if echo "$_gog_chk" | grep -q "@" && ! echo "$_gog_chk" | grep -qiE 'invalid_grant|expired or revoked'; then
        ...
        echo "  gog auth tokens export jleechan@gmail.com --out ~/.openclaw/credentials/gog-refresh-token.json"
    fi
else
