...
"
    ...
    # Linux: always install systemd service regardless of plist template existence
    _LINUX_WEBHOOK_SECRET="$(python3 -c "import json,os; p=os.path.expanduser('~/.openclaw/webhook.json'); d=json.load(open(p)) if os.path.exists(p) else {}; print(d.get('webhookSecret',''))" 2>/dev/null || true)"
    install_systemd_service "openclaw-webhook" \
        "$PYTHON_PATH_FOR_LAUNCHD $REPO_DIR/src/orchestration/webhook_daemon.py" \
        "simple" \
        "Environment=WEBHOOK_PORT=19888
Environment=WEBHOOK_DB_PATH=$HOME/.openclaw/webhook_queue.db
Environment=WEBHOOK_LOG_DIR=$HOME/.openclaw/logs
Environment=GITHUB_WEBHOOK_SECRET=$_LINUX_WEBHOOK_SECRET" \
        "webhook"
    echo "  ✓ openclaw-webhook installed (webhook daemon on port 19888)"
    WEBHOOK_INSTALLED=1
fi
