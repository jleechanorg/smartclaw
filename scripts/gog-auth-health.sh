...
# Uses GOG_KEYRING_PASSWORD from env or ~/.openclaw/openclaw.json (see lib/gog-env.sh).
#
# Exit codes:
...
load_gog_env_from_openclaw "${LIVE_OPENCLAW:-$HOME/.openclaw}/openclaw.json"
...
if echo "$auth_out" | grep -qiE 'invalid_grant|expired or revoked'; then
  echo "refresh token invalid — try: gog auth tokens import ~/.openclaw/credentials/gog-refresh-token.json" >&2
...
if echo "$drive_out" | grep -qi 'insufficientPermissions'; then
  ...
  echo "  gog auth tokens export $email --out ~/.openclaw/credentials/gog-refresh-token.json --overwrite" >&2
  exit 2
fi
