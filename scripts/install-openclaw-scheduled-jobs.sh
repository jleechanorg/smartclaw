...
#   ~/.openclaw/cron/jobs.json  -- LIVE, gitignored; gateway-managed PR automation jobs
...
declare -A MIGRATED_JOBS=(
  ["c0accca2-3b58-4da6-ba84-e8c929387e30"]="ai.openclaw.schedule.morning-log-review"
  ["4ec2aa58-5c97-4c46-8775-a7f030d1dec6"]="ai.openclaw.schedule.weekly-error-trends"
  ["95f858df-0fe8-4434-90c9-c5c89f61889e"]="ai.openclaw.schedule.docs-drift-review"
  ["d6bb3693-9f5c-4a4e-99ed-bc56eb33e35c"]="ai.openclaw.schedule.cron-backup-sync"
  ["abf80788-7bb0-4ce7-9e09-6c1a97faa5cd"]="ai.openclaw.schedule.daily-research"
)
...
for script in "${JOB_SCRIPTS[@]}"; do
  ...
  # When the repo lives at ~/.openclaw, REPO_ROOT/scripts and LIVE_DIR/scripts are the same path;
...
for plist in "$LAUNCHD_TEMPLATES_DIR"/ai.openclaw.schedule.*.plist.template; do
  [[ -f "$plist" ]] || continue
...
for plist in "$LAUNCHD_TEMPLATES_DIR"/ai.openclaw.schedule.*.plist; do
  [[ -f "$plist" ]] || continue
...
# Also install ai.openclaw.claude-memory-sync (background sync service).
# Does not use the ai.openclaw.schedule.* pattern — it's a persistent service, not
# a cron-style scheduled job — but is installed alongside them for discoverability.
# The template is named ai.openclaw.claude-memory-sync.plist.template so that
# _install_plist derives label=ai.openclaw.claude-memory-sync, matching the plist
# <string>ai.openclaw.claude-memory-sync</string> (launchctl enable/disable work).
#
...
MEMORY_SYNC_PLIST="$LAUNCHD_TEMPLATES_DIR/ai.openclaw.claude-memory-sync.plist.template"
