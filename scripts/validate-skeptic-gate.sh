...
echo "== CI external_runs filter (must exclude Green/Skeptic + Staging Canary) =="
# Fixture: one page of check-runs API shape
FIXTURE='{"check_runs":[
  {"name":"Green Gate","conclusion":null,"status":"in_progress","app":{"slug":"github-actions"}},
...
RESULT=$(echo "$FIXTURE" | jq -rs --arg now "$(date +%s)" '
  ...
  ($runs | map(select(.name != "Green Gate" and .name != "Skeptic Gate" and .name != "Staging Canary Gate"))) as $external_runs |
...
echo "OK: CI_STATUS=$RESULT (Green/Skeptic in_progress ignored for gate 1)"
...
if command -v actionlint >/dev/null 2>&1; then
  ...
  actionlint .github/workflows/green-gate.yml .github/workflows/skeptic-cron.yml 2>&1 || true
else
