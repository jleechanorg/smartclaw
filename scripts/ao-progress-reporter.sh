...
LOG_DIR="${AOPR_LOG_DIR:-${HOME}/.openclaw/logs}"
STATE_FILE="${AOPR_STATE_FILE:-$HOME/.openclaw/logs/ao-progress-state.json}"
...
post_slack() {
  ...
  local token="${OPENCLAW_SLACK_BOT_TOKEN:-}"
  if [[ -z "$token" ]]; then
    log "ERROR: OPENCLAW_SLACK_BOT_TOKEN not set"
    return
  fi
...
resolve_token() {
  local tok=""
  # Try openclaw config(s) for embedded gh token (prod may use ~/.openclaw_prod)
  for cfg in "$HOME/.openclaw/openclaw.json" "$HOME/.openclaw_prod/openclaw.prod.json"; do
    [[ -f "$cfg" ]] || continue
...
req = urllib.request.Request(
  'https://slack.com/api/chat.postMessage',
  data=payload.encode(),
  headers={'Authorization': 'Bearer ${OPENCLAW_SLACK_BOT_TOKEN}', 'Content-Type': 'application/json'},
  method='POST'
)
with urllib.request.urlopen(req) as resp:
  result = json.load(resp)
...
# ── Main ──────────────────────────────────────────────────────────────────────
{
  ...
  while IFS= read -r session_json; do
    ...
    case "$project_id" in
      ...
      jleechanclaw)       repo="jleechanorg/jleechanclaw" ;;
