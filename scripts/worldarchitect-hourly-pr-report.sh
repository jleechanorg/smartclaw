...
done < <(jq -c '.items[]' <<<"$raw" 2>/dev/null || jq -c '.[]' <<<"$raw" 2>/dev/null)

# Build summary
if (( needs_attention == 0 )) && (( green == 0 )); then
  message="• No open PRs in \`$REPO\`"
else
  message="• \`$REPO\` — 🟢 $green ready, 🔴 $needs_attention needs attention (of $total_count total)"
fi
...
if [[ -n "$SLACK_BOT_TOKEN" && "$SLACK_BOT_TOKEN" != "null" ]]; then
  curl -s "https://slack.com/api/chat.postMessage" \
    ...
    -d "$(jq -n --arg ch "$CHANNEL_ID" --arg msg "$message" --arg ts "$THREAD_TS" \
