...
# files in ~/.openclaw, stages+commits them on a feature branch, and opens/updates
...
LOG_DIR="${CPC_LOG_DIR:-${HOME}/.openclaw/logs}"
STATE_FILE="${CPC_STATE_FILE:-$HOME/.openclaw/logs/commit-pending-state.json}"
COMMIT_LOG="${CPC_COMMIT_LOG:-$HOME/.openclaw/logs/commit-pending.log}"

# Default 30 minutes; plist uses StartInterval=1800
RUN_INTERVAL_SECS="${CPC_RUN_INTERVAL_SECS:-1800}"

REPO="${CPC_REPO:-${HOME}/.openclaw}"
...
warn_untracked() {
  ...
  if [[ "${CPC_DISABLE_SLACK:-0}" != "1" ]]; then
    slack_post "⚠️ Untracked files in ~/.openclaw ($count total) — NOT auto-committed.
...
slack_post() {
  ...
  if [[ -z "$token" ]]; then
    log "WARN: no Slack token — skipping notification (set OPENCLAW_SLACK_BOT_TOKEN to enable)"
    return 0
  fi
...
run_ao_fallback() {
  ...
  local ao_task="Diagnose and fix the commit-pending-changes.sh failure in jleechanclaw.
