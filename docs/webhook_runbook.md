...
ID, and persists raw payloads to a SQLite queue (`~/.smartclaw/webhook_queue.db`).
...
3. **No FAILED events** — should return an empty list under normal load:
   ...
   PYTHONPATH=src python3 -c "
   import sqlite3, os
   db = os.path.expanduser('~/.smartclaw/webhook_queue.db')
...
3. Check Symphony logs for dispatch errors:
   ```bash
   tail -50 ~/.smartclaw/logs/gateway.log | grep -i "dispatch\|symphony\|error"
   ```
...
| Critical | DB file missing / corrupt | Restore from `~/.smartclaw/` backup; replay raw deliveries from GitHub App delivery log |
