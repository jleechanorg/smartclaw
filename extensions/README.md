...
No Mem0 key needed. Mem0 OSS defaults use OpenAI for embeddings/LLM unless you override `oss` — set `OPENAI_API_KEY` when those components use OpenAI.
...
**jleechanclaw harness default (OSS):** OpenAI **`text-embedding-3-small` @ 768** for `oss.embedder` (aligned with Qdrant), **Ollama** for `oss.llm` (e.g. `llama3.2:3b`) — no Groq for mem0 extraction. Claude hooks mirror this in `.claude/hooks/mem0_config.py`.

Fully **local** stack (Ollama embedder + LLM — no OpenAI/Groq API for those components):
...
"oss": {
  ...
  "vectorStore": {
    "provider": "qdrant",
    "config": { "host": "127.0.0.1", "port": 6333, "collectionName": "openclaw_mem0", "embeddingModelDims": 768 }
  },
  "llm": {
    "provider": "ollama",
    "config": { "model": "llama3.2:3b", "ollama_base_url": "http://localhost:11434", "temperature": 0 }
  }
}
```

Run `ollama pull nomic-embed-text` and `ollama pull llama3.2:3b` (or set `model` to whatever you pulled). Switching **only** the LLM from a remote API (e.g. Groq) to Ollama does **not** require re-embedding existing Qdrant points if the **embedder** model and dimension stay the same.
...
Everything inside `oss` is optional — upstream defaults use OpenAI embeddings (`text-embedding-3-small`), in-memory vector store, and OpenAI LLM. This repo’s harness commonly sets **Qdrant + OpenAI embeddings @ 768** and **Ollama** for `oss.llm` (see section above). Override only what you need.
